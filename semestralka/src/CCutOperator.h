...
public:
    ...
    CCutOperator(shared_ptr<CMatrix> & operand, std::size_t numRows, std::size_t numCols, pair<size_t, size_t> startPoint)
    ...
    CMatrix * Evaluate(CMemory & memory) override{
        ...
        } else if (m_StartPoint.first > m_Operand->m_NumRows - m_NumRows || m_StartPoint.second > m_Operand->m_NumCols - m_NumCols) {
